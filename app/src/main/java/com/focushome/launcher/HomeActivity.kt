    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            FocusHomeTheme {
                ...
                if (!hasPassword) {
                    SetPasswordScreen(
                        title = "Welcome to Focus Home",
                        subtitle = "Pick a password. You will need it to change which " +
                            "apps show up here, so choose something you will remember. " +
                            "There is no recovery: if you forget it, the only way back " +
                            "in is to clear this app's data from Android Settings.",
...
private fun HomeRoute(
    ...
    val weatherEnabled = remember(refreshKey) { prefs.weatherEnabled }
    val useFahrenheit = remember(refreshKey) { prefs.useFahrenheit }
    ...
    LaunchedEffect(weatherEnabled, hasLocationPermission) {
        if (weatherEnabled && !hasLocationPermission && !prefs.hasRequestedLocationPermission) {
            prefs.hasRequestedLocationPermission = true
            permissionLauncher.launch(Manifest.permission.ACCESS_COARSE_LOCATION)
        }
    }

    LaunchedEffect(refreshKey, weatherEnabled, hasLocationPermission) {
        weather = if (weatherEnabled) weatherRepository.currentWeather() else null
    }
    ...
    HomeScreen(
        ...
        onLaunch = { app ->
            ...
            if (intent == null) {
                Toast.makeText(context, "${app.label} is no longer available", Toast.LENGTH_SHORT)
                    .show()
            } else {
                try {
                    ...
                    Toast.makeText(context, "Not allowed to open ${app.label}", Toast.LENGTH_SHORT)
                        .show()
                }
            }
        },
...
private const val TAG = "FocusHome"
